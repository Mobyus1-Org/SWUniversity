...
function resolveInnateOnAttack(
  ...
  switch (sourceCardId) {
    ...
    case "JTL_149": { // Red Squadron Y-Wing — "On Attack: Deal 3 indirect damage to the DEFENDING
      ...
      const defender149: PlayerId = target149.type === "base"
        ? target149.player
    ...
    case "LAW_051": { // Beilert Valance (Target: Vader) — "On Attack: Draw a card. You may deal
                      // damage to a ground unit equal to the number of cards you've drawn this
                      // phase." The draw is MANDATORY and happens first, so it counts itself.
      DrawCardForPlayer(GetGame()!.currentGameState, GetGame()!.gameLog, attacker.controller);
      const ground051 = AllGroundUnits();
      if (ground051.length === 0) return continuation;
      const amount051 = CardsDrawnThisPhase(attacker.controller);
      if (amount051 <= 0) return continuation;
      return optionalTarget(
        "LAW_051_OA",
        attacker.controller,
        ground051.map(u => u.playId),
        `Deal ${amount051} damage to a ground unit?`,
        { yesLabel: `Deal ${amount051}`, continuation },
      );
    }
    ...
    case "SOR_047": { // Kanan Jarrus — On Attack: You may discard 1 card from the defending player's deck
      ...
      const spectreCount = GetUnitsForPlayer(attacker.controller)
        .filter(u => UnitTraits(u).includes("Spectre")).length;
    ...
    case "TWI_154": { // Mister Bones — "On Attack: If you have no cards in your hand, you may deal
                      // 3 damage to a ground unit." Any ground unit, either side.
      if (GetHand(attacker.controller).length > 0) return continuation;
      const ground154 = AllGroundUnits();
      if (ground154.length === 0) return continuation;
      return optionalTarget("TWI_154", attacker.controller, ground154.map(u => u.playId),
        "Deal 3 damage to a ground unit?", { yesLabel: "Deal 3", continuation });
    }
    case "TWI_048": { // Obi-Wan's Aethersprite — the On Attack half of its shared ability.
      return buildAethersprite(attacker.controller, attacker.playId, continuation);
    }
